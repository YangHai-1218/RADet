...
class MixDataset(ConcatDataset):
    '''
    Mix multiple training datasets, e.g. pbr images and real images, with specificed ratio
    '''
    def __init__(self, datasets, ratios) -> None:
        assert len(datasets) == len(ratios)
        repeated_datasets = [RepeatDataset(dataset, ratio) for dataset, ratio in zip(datasets, ratios)]
        super(MixDataset, self).__init__(repeated_datasets)
